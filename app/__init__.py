...
from app.utils.db_optimization import set_sqlite_pragma as set_sqlite_pragma

# Import des optimisations Python 3.13 (side-effect au chargement)
from app.utils.python313_optimizations import get_python313_info as get_python313_info
...
def create_app(config_name):
    ...
    def after_request(response):
        ...
        if elapsed_time > 2.0:
            app.logger.warning(
                f"Slow request: {request.method} {request.url} took {elapsed_time:.2f}s from {client_ip}"
            )

        # Log des requêtes très lentes (> 5 secondes) - CRITIQUE
        if elapsed_time > 5.0:
            app.logger.error(
                f"CRITICAL: Very slow request: {request.method} {request.url} took {elapsed_time:.2f}s from {client_ip}"
            )
        ...
        if response.status_code == 502:
            app.logger.error(
                f"502 Bad Gateway: {request.method} {request.url} from {client_ip} - Possible timeout or worker issue"
            )

        # Log de toutes les requêtes avec IP (pour debug)
        app.logger.info(
            f"Request: {request.method} {request.url} {response.status_code} {elapsed_time:.2f}s from {client_ip}"
        )
    ...
    def not_found_error(error):
        return render_template("errors/error.html", error_message="La page que vous recherchez n'existe pas."), 404
    ...
    def handle_exception(error):
        ...
        if isinstance(error, HTTPException):
            return render_template("errors/error.html", error_message=error.description), error.code
    ...
    def status_color_filter(status):
        ...
        status_colors = {"à faire": "info", "en cours": "warning", "terminé": "success"}
    ...
    def priority_color_filter(priority):
        ...
        priority_colors = {"basse": "secondary", "normale": "primary", "haute": "warning", "urgente": "danger"}
    ...
    def auto_archive(force):
        ...
        if current_env != "production" and not force:
            print(
                f"Archivage automatique désactivé en environnement '{current_env}'. Seul l'environnement 'production' permet l'archivage automatique."
            )
            print(f"Variables détectées - FLASK_ENV: {flask_env}, App config: {app_env}")
            print("Pour forcer l'archivage en développement, utilisez: flask auto-archive --force")
            return

        if force and current_env != "production":
            print(
                f"⚠️  Mode FORCE activé - Archivage en environnement '{current_env}' (normalement réservé à la production)"
            )
