...
class Task(db.Model):
    ...
    recurrence_template = db.relationship(
        "TaskRecurrenceSeries", foreign_keys=[TaskRecurrenceSeries.template_task_id], uselist=False, lazy=True
    )
    ...
    def clone(self, clone_checklist_items: bool = True):
        ...
        if clone_checklist_items:
            ...
            for item in self.checklist_items:
                cloned_task.checklist_items.append(
                    ChecklistItem(content=item.content, is_checked=False, position=item.position)
                )
    ...
    def log_time(self, hours, user_id, description=None):
        ...
        entry = TimeEntry(task_id=self.id, user_id=user_id, minutes=minutes, description=description)
    ...
    def archive(self):
        ...
        self.archived_at = datetime.now(UTC).replace(tzinfo=None)
