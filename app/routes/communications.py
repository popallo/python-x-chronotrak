...
def get_communication_stats():
    ...
    return {
        ...
        "last_24h": Communication.query.filter(Communication.sent_at >= now - timedelta(days=1)).count(),
        "last_7d": Communication.query.filter(Communication.sent_at >= now - timedelta(days=7)).count(),
    }
...
def view_communication(comm_id):
    ...
    return render_template(
        "admin/communication_detail.html",
        comm=comm,
        user=user,
        task=task,
        project=project,
