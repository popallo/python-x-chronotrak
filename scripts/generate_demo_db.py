...
try:
    ...
    print("Vérifiez que les fichiers de l'application sont corrects.")
...
def generate_admin_account(admin_email, admin_password):
    ...
    admin = User(name="Administrateur", email=admin_email, role="admin")
...
def generate_users():
    ...
    client_users = []
    for _i in range(NUM_CLIENT_USERS):
        first_name = random.choice(FIRST_NAMES)
        last_name = random.choice(LAST_NAMES)
        name = f"{first_name} {last_name}"
        ...
        client_user = User(name=name, email=email, role="client")
    ...
    for client in clients:
        ...
        for i in range(num_projects):
            ...
            credit_log = CreditLog(
                project_id=project.id, amount=initial_credit, note="Crédit initial", created_at=created_at
            )
        ...
        if task.status in ["en cours", "terminé"] and task.actual_minutes:
            ...
            for _i in range(num_entries):
                ...
                time_entry = TimeEntry(
                    task_id=task.id, user_id=user.id, minutes=minutes, description=description, created_at=created_at
                )

                db.session.add(time_entry)

            print(
                f"Entrées de temps générées pour la tâche {task.id}: {num_entries} entrées pour {total_time_needed_minutes} min"
            )
...
def generate_comments(tasks, users, client_users):
    ...
    for task in tasks:
        ...
        for _i in range(num_comments):
            ...
            comment = Comment(content=content, task_id=task.id, user_id=user.id, created_at=created_at)
...
def create_database(db_path=None):
    ...
    with app.app_context():
        try:
            ...
            # Générer un admin
            generate_admin_account(admin_email, admin_password)
